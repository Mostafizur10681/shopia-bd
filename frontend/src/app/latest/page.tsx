...
export default function LatestProductsPage() {
  ...
  const latestProducts = productsData.filter((p) => p.isNew);
  const categories = ["All", ...Array.from(new Set(latestProducts.map((p) => p.category)))];

  const filteredProducts = latestProducts.filter((p) => {
